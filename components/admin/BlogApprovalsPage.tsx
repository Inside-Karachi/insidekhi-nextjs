...
> = {
  blue: {
    ...
    title: "text-sm font-medium text-blue-900 dark:text-blue-100 group-hover:text-blue-800 dark:group-hover:text-blue-200 transition-colors",
    value: "text-2xl font-bold text-blue-700 dark:text-blue-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors",
    iconWrapper: "p-2 bg-blue-500/10 rounded-lg group-hover:bg-blue-500/20 transition-colors",
  ...
  orange: {
    ...
    title: "text-sm font-medium text-orange-900 dark:text-orange-100 group-hover:text-orange-800 dark:group-hover:text-orange-200 transition-colors",
    value: "text-2xl font-bold text-orange-700 dark:text-orange-300 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors",
    iconWrapper: "p-2 bg-orange-500/10 rounded-lg group-hover:bg-orange-500/20 transition-colors",
  ...
  green: {
    ...
    title: "text-sm font-medium text-green-900 dark:text-green-100 group-hover:text-green-800 dark:group-hover:text-green-200 transition-colors",
    value: "text-2xl font-bold text-green-700 dark:text-green-300 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors",
    iconWrapper: "p-2 bg-green-500/10 rounded-lg group-hover:bg-green-500/20 transition-colors",
  ...
  red: {
    ...
    title: "text-sm font-medium text-red-900 dark:text-red-100 group-hover:text-red-800 dark:group-hover:text-red-200 transition-colors",
    value: "text-2xl font-bold text-red-700 dark:text-red-300 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors",
    iconWrapper: "p-2 bg-red-500/10 rounded-lg group-hover:bg-red-500/20 transition-colors",
...
export function BlogApprovalsPage() {
  ...
  const fetchPosts = React.useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/blogs/approvals?${params.toString()}`);
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: result.error || "Failed to load blog approvals",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to load blog approvals",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const response = await fetch("/api/admin/blogs/approvals", {
        method: "POST",
  ...
  const getStatusBadge = (status: PostStatus) => {
    const config = {
      draft: { variant: "secondary" as const, label: "Draft", icon: FileEdit },
      pending_approval: {
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="relative">
        ...
        <div className="relative p-6">
          ...
          <p className="text-muted-foreground text-lg">
            Review and approve writer blog submissions before they go live.
          </p>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          { key: "pending", label: "Pending Review", value: stats.pendingCount, icon: Clock, color: "orange" as StatVariant },
          { key: "published", label: "Published", value: stats.publishedCount, icon: CheckCircle2, color: "green" as StatVariant },
          { key: "rejected", label: "Rejected", value: stats.rejectedCount, icon: XCircle, color: "red" as StatVariant },
          { key: "drafts", label: "Drafts", value: stats.draftCount, icon: FileEdit, color: "blue" as StatVariant },
        ].map((card) => {
      ...
      >
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Button variant="outline" onClick={fetchPosts} disabled={loading} className="h-11">
            ...
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                {statusFilter === "pending_approval"
                  ? "All post submissions have been processed. New submissions will appear here."
                    ...
                    <div className="flex items-center justify-between p-6">
                      <div className="flex-1 space-y-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              <div className="flex items-center gap-1.5">
                                Author: {post.author_full_name || "Unknown"}
                              </div>
                        ...
                        {post.submitted_at && (
                          <div className="flex items-center gap-2 text-xs text-muted-foreground pl-12">
                            ...
                            <span>Submitted {format(new Date(post.submitted_at), "PPp")}</span>
                          </div>
                        )}
                        ...
                        {post.review_notes && (
                          <div className="pl-12 mt-2 p-3 bg-muted/50 rounded-lg">
                            ...
                            <p className="text-sm">{post.review_notes}</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => window.open(`/guides/${post.slug}`, "_blank")}
                          className="h-9"
                        >
              ...
              <Textarea
                placeholder={
                  reviewAction === "approve"
                    ...
                    : "Please explain why this post is being rejected..."
                }
                value={reviewNotes}
