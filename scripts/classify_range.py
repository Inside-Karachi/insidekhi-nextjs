...
def classify_row(row):
    ...
    semantic_patterns = [
        # Professional Services / Financial / Banks (HIGH PRIORITY)
        (r"\b(bank|banking|islamic bank|finance|financial|microfinance|debit card|credit card|loan|personal loan|auto finance|home finance)\b", "professional-business-services", 0.95),
        ...
        (r"\b(grocery|groceries|fresh vegetables|fruits|frozen food|meat shop|fresh meat|supermarket|mart|super market|milk|cow milk|dairy|dairy farm|butter|ghee)\b", "groceries-fresh-food", 0.9),

        # Apparel & Clothing
        (r"\b(garment|garments|clothing|apparel|mens wear|men's wear|ladies|women's wear|kids wear|baby shop|denim|dresses|bridal dress|party wear|casual dresses|collection|collections|lawn|boutique|tailor|unstitched|fashion studio|couture|chikankari)\b", "apparel-clothing", 0.9),

        # Footwear & Bags
        (r"\b(shoes|footwear|bags|handbags|leather bags|slippers|sandals)\b", "footwear-bags", 0.9),

        # Jewelry & Watches
        (r"\b(jewelry|jewellery|jewelers|jewellers|watch shop|watches)\b|\b(gold|diamond|silver)\s+(jewel|jewelry|ring|bangle|necklace|chain|ornament|shop|store|showroom)\b", "jewelry-watches", 0.9),
        ...
        (r"\b(shopping mall|mall|plaza|trade center|outlet|ice rink)\b", "shopping-malls-outlets", 0.85),
        ...
        # Health & Wellness
        (r"\b(medical store|pharmacy|chemist|online pharmacy|emeds)\b", "pharmacies-medical-stores", 0.9),
        (r"\b(hospital|hospitals|clinic|clinics|labs|laboratory|blood bank|diagnostic|physician|doctor|infertility)\b", "clinics-hospitals", 0.9),
        ...
        # Travel & Tourism
        (r"\b(tour|tours|tourist|travel|tourism|hotel|hotels|resort|adventure tours|retreats|guest house|motel|hajj|umrah|visa service|airline|airlines)\b", "travel-tourism", 0.9),
        ...
        # Schools & Education
        (r"\b(school|schools|montessori|preschool|grammar school|academy|secondary school)\b", "schools-pre-schools", 0.9),
...
def main():
    start_idx = int(sys.argv[1]) if len(sys.argv) > 1 else 5000
    end_idx = int(sys.argv[2]) if len(sys.argv) > 2 else 6000
    output_path = sys.argv[3] if len(sys.argv) > 3 else "/Volumes/umr_drive/Documents/insidekhi-nextjs/exports/classified_listings_5001_to_6000.csv"
    ...
    print(f"\nDone! Exported {len(classified_results)} classified listings to: {output_path}")
